        ...
        foreach ($fieldData as $key => $value) {
            ...
            switch ($key) {
                ...
                case 'licenceNumber':
                    ...
                    }else if(($this->checkData('service_registrations','licence_number',$value))){
                        $this->errArray[$key] = ' already added Licence Number';
                    }
                    break;
                case 'vehicleNumber':
                    if ((!preg_match('/^[A-Z]{2}[ -][0-9]{1,2}(?: [A-Z])?(?: [A-Z]*)? [0-9]{4}$/',$value))) {
                        $this->errArray[$key] = 'Enter valid Vehicle Number';
                    }else if(($this->checkData('service_registrations','vehicle_number',$value))){
                        $this->errArray[$key] = 'already added vehicle Number';
                    }
                    break;  
            }
        }  
